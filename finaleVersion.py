...
if "shared_state" not in st.session_state:
    st.session_state.shared_state = {"selected_option": None, "selectbox_shown": False, "main_aufruf": False, "should_restart_main_async": False}
...
async def popup_message():
    ...
    if not st.session_state.shared_state["selectbox_shown"]:
        with st.sidebar:
            ...
            selected_option = selectbox_placeholder.selectbox("!WARNING! Die Bodenfeuchte liegt außerhalb des optimalen Bereichs. Es wird empfohlen einen neuen Plan erstellen zu lassen. Soll ein neuer erstellt werden?", ["","JA", "NEIN"], key = np.random.randint(0,100000))
    ...
    try:
        ...
        while st.session_state.i < 88:
            ...
            if selected_option == "JA":
                print("break")
                #st.session_state.dfIrrigation = pd.DataFrame(columns=st.session_state.dfIrrigation.columns)
                # Simuliere das Auswählen der "x"-Spalte programmgesteuert
                ...
                bwPlan_aktualisiert = True
                st.session_state.shared_state["selected_option"] = None
                st.session_state.shared_state["selectbox_shown"] = False 
                st.session_state.shared_state["main_aufruf"] = True
                selectbox_placeholder.empty()

            if bwPlan_aktualisiert == True:
                time.sleep(4)
            ...
            if new_value < 59 or new_value > 81 and not st.session_state.shared_state["selectbox_shown"]:
                print("popup_message")
                await asyncio.create_task(popup_message())
                st.session_state.shared_state["selectbox_shown"] = True

            elif new_value < 69 or new_value > 81 and st.session_state.i < 35: #optimaler Bereich letzten beiden Phasen
                print("popup_message")
                await asyncio.create_task(popup_message())
                st.session_state.shared_state["selectbox_shown"] = True
